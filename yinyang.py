...
class Yinyang(BaseEstimator, ClusterMixin):
    """
    ...
    def fit(self, X):
        ...
        shifted_samples = np.zeros(self.n_clusters)
        sample_array = np.arange(X.shape[0])
        ...
        self.upper_and_lower_bounds = self.distances_[sample_array.reshape(-1, 1), upper_and_lower_bounds]
        ...
        for n_iter in range(self.max_iter):
            ...
            for i in range(self.n_clusters):
                mask = self.labels_ == i
                self.upper_and_lower_bounds[:, 0][mask] += drift[i]
                self.upper_and_lower_bounds[:, 1][mask] -= drift[i]
            ...
            mask_changed_bounds = self.upper_and_lower_bounds[:, 1] < self.upper_and_lower_bounds[:, 0]
            ...
            for i in range(self.n_clusters):
                ...
                self.upper_and_lower_bounds[:, 0][new_mask] = distances
            ...
            self.upper_and_lower_bounds[:, 1][mask_changed_bounds] = self.upper_and_lower_bounds[:, 0][mask_changed_bounds]

            #XXX: Vectorize?
            for i in range(self.n_clusters):
                cluster = self.cluster_centers_[i]
                new_mask = np.logical_and(mask_changed_bounds, self.labels_ == i)
                shifted_samples[i] = np.count_nonzero(new_mask)
                distances = np.sum((X[new_mask] - cluster)**2, axis=1)
                self.upper_and_lower_bounds[:, 0][new_mask] = distances
